...
class SentimentAnalyzer:
    ...
    def __init__(self, pretrained=False):
        ...
        if pretrained:
            ...
            self.vocabulary = set()
            self.priors = {}
            self.class_to_word_to_likelihood = {}
    ...
    def train(self, train_set_data: tuple[str, str]):
        """
        Trains the model on the given training set. 
        """
        classes = zip(*train_set_data)[1]
        self.create_vocabulary(zip(*train_set_data)[0])
        for sentiment in set(classes):
            prior = classes.count(sentiment)/len(train_set_data)
            self.priors[sentiment] = (prior, math.log(prior))
            reviews = [review for review in train_set_data if review[1] == sentiment]
